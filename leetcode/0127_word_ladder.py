...
class Solution:
    ...
    def get_patterns(word):
        ...
        for j in range(len(word)):
            res.append(word[:j] + "*" + word[j + 1 :])
        return res

    def ladderLength(self, beginWord: str, endWord: str, wordList: List[str]) -> int:
        # build graph where words are connected if they differ by 1 letter
        adj_list = defaultdict(list)  # Time O(n m^2)
        wordList += [beginWord]
        for word in wordList:
            patterns = self.get_patterns(word)
            for p in patterns:
                adj_list[p].append(word)
        ...
        while queue:
            ...
            for _ in range(len(queue)):
                ...
                patterns = self.get_patterns(curr_word)
                for p in patterns:
                    for neigh in adj_list[p]:
